...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		switch typeName {
		case "Author":
			resolverName, err := entityResolverNameForAuthor(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Author": %w`, err)
			}
			switch resolverName {
			...
			case "findAuthorByID":
				...
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findAuthorByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindAuthorByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Author": %w`, err)
				}
...
func entityResolverNameForAuthor(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
		...
		return "findAuthorByID", nil
	}
	return "", fmt.Errorf("%w for Author", ErrTypeNotFound)
}

func entityResolverNameForBook(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
			m   map[string]interface{}
			val interface{}
		...
		return "findBookByID", nil
	}
	return "", fmt.Errorf("%w for Book", ErrTypeNotFound)
}
